...
import { saveEnterpriseSettingsData } from "@/lib/api/client";
...
export function EnterpriseSettingsWorkbench({
  initialData,
}: {
  initialData: {
  ...
  const save = async () => {
    ...
    try {
      ...
      const result = await saveEnterpriseSettingsData(null, { roleRows, governanceRows });
      setProgress(100);
      setHint(result.saved ? "企业设置已保存到数据库" : "保存请求已提交，但数据库不可用");
    } catch {
